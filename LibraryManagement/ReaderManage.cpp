...
void UpdateReaderInfo(list <Reader> &l, string id)
{
	...
	for (i = l.begin(); i != l.end(); i++)
	{
		...
		{
			DisplayReaderInfo(temp);
			temp = ChangReaderInfo(temp);
			i->Info = temp.Info;
			UpdateReaderFile(l);
		}
	}
...
Reader ChangReaderInfo(Reader reader)
{
	...
	if (check == (int) 'y')
	{
		...
		cout << "Gioi tinh (Nam-1 / Nu-0) - " << endl;
		check = _getch();
		if (check == (int)'y')
		{
			cout << "Gioi tinh                : "; DisplaySex(temp.Info.Sex); cout << endl;
...
int SearchCitizenID(list <Reader> l, string id)
{
	...
	for (i = l.begin(); i != l.end(); i++)
	{
		...
		temp.ID = i->ID;
		temp.Info = i->Info;
		if (id.compare(temp.Info.CitizenId) == 0)
		{
			DisplayReaderInfo(temp);
			check = 0;
...
int SearchName(list <Reader> l, string name)
{
	...
	for (i = l.begin(); i != l.end(); i++)
	{
		...
		temp.ID = i->ID;
		temp.Info = i->Info;
		if (temp.Info.Name.find(name, 0) != string::npos)
		{
			DisplayReaderInfo(temp);
			check = 0;
		}
